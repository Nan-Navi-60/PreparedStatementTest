...
1. ["우리의 애플리케이션에서 PreparedStatement는 어떻게 동작하고 있는가", 카카오페이 아티클](https://tech.kakaopay.com/post/how-preparedstatement-works-in-our-apps/)에 따르면,  
...
위 가설에 기반하여 쿼리를 보다 복잡하게 구성한 케이스는 다음과 같음
...
[Testing.java](https://github.com/Nan-Navi-60/PreparedStatementTest/blob/main/src/main/java/jdbc/Testing.java#L15)
...
실험 결과, 평균 수행 시간을 비교했을 때, 두 옵션이 모두 true인 경우가 가장 빨랐고, `useServerPrepStmts`는 true이고 `cachePrepStmts`는 false인 경우가 가장 느렸다.
...
본 실험에서는 Prepared Statement의 생성 위치(**Server vs Client**)와 캐싱 전략에 따라 수행 시간이 각기 다른 양상으로 나타남을 확인했다.

* **Server-side Prepared Statement**
    * **캐싱 활성 시**: SQL 서버 측에 Statement가 생성 및 캐싱되어, 동일 쿼리 요청 시 재사용되므로 수행 시간이 가장 짧고 효율적이다.
    * **캐싱 비활성 시**: 클라이언트의 요청마다 Statement 생성과 삭제를 반복하는 과정이 추가되어 수행 시간이 급격히 길어지는 현상을 보였다.
* **Client-side Prepared Statement**
    * 실험 결과, 오히려 캐싱을 적용했을 때 수행 시간이 더 길게 측정되었다. 이를 통해 **CPU가 직접 SQL문을 파싱하여 서버에 전달하는 비용보다, 캐싱 과정에서 발생하는 메모리 참조 오버헤드가 더 크다**는 결론을 도출했다.
...
* **검증 결과:**
  Server와 Client 양측 모두 캐시 사용 여부에 따른 유의미한 편차를 확인했다. 다만, Server 측에서는 캐싱이 성능을 향상시키는 반면, Client 측에서는 오히려 오버헤드로 작용하여 성능 차이가 반대 양상으로 나타남을 확인했다.

#### **가설 2: "JOIN이 포함된 복잡한 쿼리일수록 Server-side Prepared Statement 캐시가 성능 향상에 크게 기여할 것이다."**
* **검증 결과:**
  선행 연구의 단순 쿼리 테스트 결과와 비교했을 때, 복잡한 JOIN 쿼리를 사용한 본 실험에서 각 케이스별 성능 편차가 훨씬 두드러지게 나타났다.
